<template>
  <div class="Register" style="width: 100%">
    ...
    <group>
        ...
        <x-input title="" type="password" placeholder="请输入新密码" v-model="password" :min="6" :max="16" @on-change="change" required></x-input>
        <x-input title="" type="password" placeholder="请再次输入新密码" v-model="passwordcopy" :min="6" :max="16"  required></x-input>
    ...
    <group>
      ...
      <box gap="10px 25px">
        <x-button type="primary" @click.native="handlechange">修改密码</x-button>
    ...
    methods:{
      ...
      handlechange(){
        ...
        else if(this.password !==this.passwordcopy)
        {
          ...
          this.$axios.post('/api/replaceUserpassword',readyData).then((res) => {
              console.log(this.birthday)
            if (res.data.status === 1){
              this.type = 'success'
              this.toastText = '修改密码成功'
              this.show1 = true
            } else {
              this.type = 'warn'
              this.toastText = '修改密码失败,资料验证失败'
              this.show1 = true
            } 
