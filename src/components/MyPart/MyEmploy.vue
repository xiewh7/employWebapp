<template>
  <div>
    <x-header >我的招聘</x-header>
    <div style="margin: 10px;overflow: hidden; font-size:1.5rem" v-for="item in list" :key="item.e_id" @click="toChart(item.u_accept_id,item.e_recent)">
...
export default {
  ...
  mounted () {
      this.$axios.all([
          this.$axios.get('/employ/queryEmploy',{params: {u_id: store.fetchId() }}),
          this.$axios.get('/api/getAllName')]).then(this.$axios.spread( (resemploy,resname) => {
            ...
            for (let i = 0;i < sortlist.length ; i++) {
                 for (let j = 0;j < namlist.length ; j++){
                     ...
                     if (sortlist[i].u_accept_id === namlist[j].u_id)
                     {
                         title = namlist[j].u_name
                         
                     }    
                };
                if (idList.indexOf(sortlist[i].u_accept_id) === -1) {
                        idList.push(sortlist[i].u_accept_id)
                        this.list.push({
                            e_id: sortlist[i].e_id,
                            e_recent:sortlist[i].e_recent,
                            title: title,
                            u_accept_id : sortlist[i].u_accept_id
                        })
                        //break
                    }
